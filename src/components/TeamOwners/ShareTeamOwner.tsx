...
const ShareTeamOwner = React.forwardRef<any, any>(({teamOwner}, ref) => {
    ...
    return (
            <div ref={ref} className="team_owner_card">
                ...
                <div>
                    {teamOwner?.digital_id && <div className="team_owner_card-id"><span className="cherryred">ID:</span> {(""+teamOwner.digital_id).substr(0, 3)+"-"+(""+teamOwner.digital_id).substr(3, 3)}</div>}
